...
Item {
    ...
    readonly property bool shouldBeActive: Config.background.visualiser.enabled && !PowerSaving.pauseVisualisers && (!Config.background.visualiser.autoHide || (Hypr.monitorFor(screen)?.activeWorkspace?.toplevels?.values.every(t => t.lastIpcObject?.floating) ?? true))
